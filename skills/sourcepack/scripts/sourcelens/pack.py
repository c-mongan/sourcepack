...
def build_pack(run,destination,include_originals=True):
    ...
    with tempfile.TemporaryDirectory(dir=dest.parent,prefix='.sourcepack-export-') as td:
        stage=Path(td)/'pack';stage.mkdir(mode=0o700);register=[];text_sections=[];frames=[];observations=[];omissions=[];gaps=[];number=0
        ...
        for frame in frames:answer=answer.replace(frame['id'],f"[{frame['id']}]({frame['path']})")
        ...
        if frames and shutil.which(w.command('ffmpeg')[0]):
            ...
            for offset in range(0,len(frames),12):
                ...
                with tempfile.TemporaryDirectory() as images:
                    ...
                    if proc.returncode:gaps.append('Contact sheet generation failed; use native images')
